...
# 반드시 설정 필요
####################################################
config_file: str = 'configs/mask_rcnn/mask_rcnn_r50_fpn_fp16_1x_coco_resistor_color.py'
checkpoint_file: str = '../latest.pth'
model: mmdet.models.detectors.mask_rcnn.MaskRCNN = None
...
# 저항띠를 추출하는 모델을 로드한다.
####################################################
def load_resistor_color_detection_model() -> mmdet.models.detectors.mask_rcnn.MaskRCNN:
  global model
  global config_file
  ...
  if model == None:
    ...
    #model = init_detector(config_file, checkpoint_file) # without GPU
  return model
...
def result_to_bands(result: list) -> List[Band_xy]:
  bands = list()
  for index, lines in enumerate(result):
    for line in enumerate(lines):
      ...
      y2 = line[3]
      acc = line[4]
      bands.append(Band_xy(ResistorColor(index), (x1+x2)/2, (y1+y2)/2, acc))
  return bands
...
def sort_band(bands:List[Band_x]) -> List[Band_x]:
  side_count = reduce(lambda val,band: val+1 if is_side(band.color) else val, bands, 0)
